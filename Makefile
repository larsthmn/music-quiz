...
clean:
	...
	powershell "if (Test-Path release/public) { Remove-Item release/public -Recurse }"
	
.PHONY: release	debug

release: clean 
	powershell "((Get-Content -path shared/spotify_template.json -Raw) -replace '\[PORT\]','80') | Set-Content -Path backend/spotify.json"
